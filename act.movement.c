...
*/
{
	...
	if (!IS_AFFECTED(ch, AFF_SNEAK)) {
	  ...
	  if ((GET_LEVEL(ch)>=21)&&(!IS_NPC(ch)))
	    if(IS_SET(ch->specials.act,PLR_GODINVIS))
	      sprintf(tmp,"You sense a drift of energy to the %s.",dirs[cmd]);
	    else sprintf(tmp,"$n levitates %s.",dirs[cmd]);
	  else sprintf(tmp, "$n leaves %s.", dirs[cmd]);
	...
	if (!IS_AFFECTED(ch, AFF_SNEAK))
	    {
	      if (IS_SET(ch->specials.act,PLR_GODINVIS))
		act("You sense a splurge of energy in the room.", TRUE, ch, 0,0, TO_ROOM);	
	      else act("$n has arrived.", TRUE, ch, 0,0, TO_ROOM);	
	    }

	do_look(ch, "\0",15);
	if (IS_SET(ch->specials.act,PLR_HUNT)) huntstep(ch,was_in);
...
void do_enter(struct char_data *ch, char *argument, int cmd)
{
	...
	{
	  if ((tmp_obj = get_obj_in_list_vis(ch,buf,world[ch->in_room].contents))&&(GET_ITEM_TYPE(tmp_obj) == ITEM_PORTAL))
	    {
	      
	      act("$n enters $p and dematerializes.",FALSE, ch,tmp_obj,0,TO_ROOM);
	      act("You enter $p and dematerialize.",FALSE, ch,tmp_obj,0,TO_CHAR);
	      char_from_room(ch);
	      for (location=0;location <=top_of_world;++location)
		  if (world[location].number == tmp_obj->obj_flags.value[0])
		    break;

	      char_to_room(ch,location);
	      act("$n materializes in the $p and steps out.",FALSE, ch,tmp_obj,0,TO_ROOM);
	      act("You rematerialize and step out.",FALSE, ch,tmp_obj,0,TO_CHAR);
...
void do_push(struct char_data *ch, char *argument, int cmd)
{
  ...
  char direction[50];
  void do_move(struct char_data *ch, char *argument, int cmd);
  half_chop(argument,name,direction);


  if (!*name || !*direction)
    {
      send_to_char("Push whom, and in what direction (north east south west up down)?\n\r",ch);
      return;
    }
  if (!(vict = get_char_room_vis(ch, name)))
    {
      send_to_char("Push whom? You don't see that person.\n\r",ch);
      return;
    }
  if (vict==ch)
    {
      send_to_char("Why are you pushing yourself? Weird...\n\r",ch);
      return;
    }
  if (!str_cmp(direction,"north")) dirint = 1;
  else if (!str_cmp(direction,"east"))  dirint = 2;
  else if (!str_cmp(direction,"south")) dirint = 3;
  else if (!str_cmp(direction,"west")) dirint = 4;
  else if (!str_cmp(direction,"up")) dirint = 5;
  else if (!str_cmp(direction,"down")) dirint = 6;
  else 
    {
      send_to_char("Push in what direction? (north, east, south, west, up, down)\n\r",ch);
      return;
    }

  if ((GET_MOVE(ch) <=15)&&(GET_LEVEL(ch)<21))
    {
      send_to_char("Pushing an unwilling person required more energy than you currently have.\n\r",ch);
      return;
    }

  sprintf(buf,"$n attempts to push $N %s.",direction);
  act(buf,TRUE,ch,0,vict,TO_NOTVICT);
  sprintf(buf,"$n attempts to push you %s.",direction);
  act(buf,TRUE,ch,0,vict,TO_VICT);
  sprintf(buf,"You attempt to push $N %s.",direction);
  act(buf,TRUE,ch,0,vict,TO_CHAR);

  if (GET_LEVEL(ch)<21) GET_MOVE(ch) -= 15;

  if (((GET_STR(ch)>GET_STR(vict)) || number(1,2)==2))
    {
      send_to_char("You failed in your resistance.\n\r",vict);

      send_to_char("You succeed.\n\r",ch);

      do_move(vict,direction,dirint);
      return;
    }
  send_to_char("You failed.\n\r",ch);
  send_to_char("You resisted.\n\r",vict);
