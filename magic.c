...

{
  ...
  if (obj) {
    sprintf(buf,"obj %d",obj_virtual(obj->item_number));
  ...
  else 
    {
      if (IS_NPC(victim))
	{
	  sprintf(buf,"char %d",mob_virtual(victim->nr));
	  do_load(ch,buf,0);
	}
      else clone_char(victim);
  ...
  struct char_data *victim, struct obj_data *obj)
{
  ...
  /* abel to be "recharmed" with duration                              */
	...
	if (!IS_SET(victim->specials.act,PLR_KILLER) &&
	  ...
	  {
	    send_to_char("You can only charm players who is a killer or thief\n\r",ch);
	    return;
	  }
      }
