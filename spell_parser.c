...
void clone_char(struct char_data *ch)
{
	...
	clone->player.short_descr = strdup(GET_NAME(ch));
	free(clone->player.long_descr);
	clone->player.long_descr = strdup(buf);
	...
	if (ch->player.description)
	  {
	    ...
	    clone->player.description = strdup(ch->player.description);
	  }
...
void add_follower(struct char_data *ch, struct char_data *leader)
{
	...
	send_to_char("You quit your previous group.\n\r",ch);
	ch->master = leader;
...
void assign_spell_pointers(void)
{
	...
	SPELLO( 9,12,POSITION_STANDING,15, 21, 40,TAR_OBJ_INV|TAR_OBJ_EQUIP|
