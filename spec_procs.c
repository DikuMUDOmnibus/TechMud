    ...
    case '3' :
      ...
      break;
		...
		case 'W' :
			...
			act("$n suddenly turns on, stands.",FALSE,ch,0,0,TO_ROOM);
			break;

		case 'S' :
			GET_POS(ch) = POSITION_SLEEPING;
			act("$n lies down and turns off.",FALSE,ch,0,0,TO_ROOM);
			break;

    case 'a' :
      act("$n says 'I'll be back!'",FALSE,ch,0,0,TO_ROOM);
      act("$n smirks.",FALSE,ch,0,0,TO_ROOM);
      break;

    case 'b' :
      act("$n says 'A nice city indeed.'",
        FALSE,ch,0,0,TO_ROOM);
      break;

    case 'c' :
      act("$n says 'I should destroy vandals.'",
        FALSE,ch,0,0,TO_ROOM);
      break;

    case 'd' :
      act("$n says 'Good day, citizens of Technopolis.'", FALSE, ch, 0,0,TO_ROOM);
      break;

    case 'e' :
      strcpy(buf,"I hereby declare Technopolis open!");
      do_shout(ch,buf,0);
      break;

    case 'E' :
      strcpy(buf,"I hereby declare Technopolis closed!");
      do_shout(ch,buf,0);
...
int guild_guard(struct char_data *ch, int cmd, char *arg)
{
	...
	strcpy(buf,  "The cybernoid humiliates you, and block your way.\n\r");
	strcpy(buf2, "The cybernoid humiliates $n, and blocks $s way.");
...
int puff(struct char_data *ch, int cmd, char *arg)
{
  ...
  switch (number(0, 60))
    {
    case 0:
      do_say(ch, "I hate being a replicant.", 0);
      return(1);
    case 1:
      do_say(ch, "Do I look human enough?", 0);
      return(1);
    case 2:
      do_say(ch, "I wish that I am not a replicant.", 0);
      return(1);
    case 3:
      do_say(ch, "I wish I was not created, but born.", 0);
      return(1);
    default:
      for (dd = world[ch->in_room].contents; dd; dd = dd->next_content) 
	{
	  if (GET_ITEM_TYPE(dd)==ITEM_PORTAL)
	    if ((number(1,10)==5)&&(GET_POS(ch)!=POSITION_FIGHTING))
	      {
		strcpy(buf,fname(dd->name));
		do_enter(ch,buf,0);
		return(FALSE);
	      }
...
for (i = world[ch->in_room].contents; i; i = i->next_content) 
  {
	...
	((i->obj_flags.type_flag == ITEM_DRINKCON) ||
	 (i->obj_flags.cost <= 10))) 
      {
	act("$n picks up some items and cleans the floor.", FALSE, ch, 0, 0, TO_ROOM);
      
	obj_from_room(i);
	obj_to_char(i, ch);
	return(TRUE);
      }
    if (GET_ITEM_TYPE(i)==ITEM_PORTAL)
      if ((number(1,10)==5)&&(GET_POS(ch)!=POSITION_FIGHTING))
	{
	  strcpy(buf,fname(i->name));
	  do_enter(ch,buf,0);
	  return(FALSE);
	}
...
max_evil=1000;
  ...
  for (tch=world[ch->in_room].people; tch; tch = tch->next_in_room) 
    {
      if ((!IS_NPC(tch)) &&(IS_SET(tch->specials.act,PLR_THIEF)
  ...
  if (evil)
    {
      act("$n screams 'Evil intruder!  Prepare to die!'", FALSE, ch, 0, 0, TO_ROOM);
...
int terminator(struct char_data *ch, int cmd, char *arg)
{
  struct obj_data *dd;
  ...
  for (i = descriptor_list; i; i = i->next)
	...
	if ((!IS_NPC(i->character)) &&
	    (IS_SET(i->character->specials.act,PLR_THIEF) ||
	     IS_SET(i->character->specials.act,PLR_KILLER)))
	  {
	    if (GET_ALIGNMENT(i->character) < max_evil)
	      {
		max_evil = GET_ALIGNMENT(i->character);
		evil = i->character;
	      }
	  }
      }
  ...
  if (evil)
    if (evil->in_room != NOWHERE) 
      {
	if ((evil->in_room)!=(ch->in_room))
	  {
	    huntmove(ch,evil);
	    return(TRUE);
	  }
	act("$n screams 'I have hunted you down! Prepare for termination!'", FALSE, ch, 0, 0, TO_ROOM);
	hit(ch, evil, TYPE_UNDEFINED);
	return(TRUE);
      }
...
if (IS_SET(ch->specials.act,PLR_NEWBIE))
  {
    strcpy(buf,"newbie_item");
    REMOVE_BIT(ch->specials.act,PLR_NEWBIE);
  }
else if (IS_SET(ch->specials.act,PLR_AWARD))
  {
    strcpy(buf,"award_item");
    REMOVE_BIT(ch->specials.act,PLR_AWARD);
  }
else
  {
  ...
  for (tmp_obj = world[key_room].contents; tmp_obj; tmp_obj = tmp_obj->next_content)
    for(ext = tmp_obj->ex_description; ext; ext = ext->next)
      if (str_cmp(buf, ext->keyword) == 0) {
		  if (gold == 0) {
		     gold = 1;
			   act("$n kneels and at the altar and chants a prayer to Zeus.",
					 FALSE, ch, 0, 0, TO_ROOM);
				act("You notice a faint light in Zeus's eye.",
