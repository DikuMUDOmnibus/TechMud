...
int maindup(int argc, char **argv)
{
	int port;
	char buf[512];
	int pos = 1;
			...
			if (point->prompt_mode)
			{
				if (point->str)
					...
					if (point->showstr_point)
					  ...
					  {
					    if (IS_SET(point->character->specials.act,PLR_PROMPT))
					      {
						if (IS_SET(point->character->specials.act,PLR_GRAPHICS))
						sprintf(tempbuf,"<%c%c%c%c%d%c%c%ch %c%c%c%c%d%c%c%cm %c%c%c%c%d%c%c%ce> ",27,91,49,109,GET_HIT(point->character),27,91,109,27,91,49,109,GET_MANA(point->character),27,91,109,27,91,49,109,GET_MOVE(point->character),27,91,109);
else						sprintf(tempbuf,"<%dh %dm %de> ",GET_HIT(point->character),GET_MANA(point->character),GET_MOVE(point->character));
						write_to_descriptor(point->descriptor, tempbuf );
...
int new_descriptor(int s)
{
	...
	if (wizlock)
	{
		write_to_descriptor(desc, "The game is godlocked...");
		close(desc);
		return(0);
	}
	...
	{
		strcpy(thehostname, (char *)inet_ntoa(sock.sin_addr));
	}
	else
	{
		strncpy(thehostname, from->h_name, 49);
		*(thehostname + 49) = '\0';
	}

	if (checkban(thehostname))
	  {
