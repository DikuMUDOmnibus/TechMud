...
void do_god(struct char_data *ch, char *argument, int cmd)
{
  ...
  if (GET_LEVEL(ch)>=21)
    {
      send_to_char("You are a god! No need to seek yourself for help.\n\r", ch); 
      return;

    }
  if (IS_SET(ch->specials.act,PLR_NOSHOUT))
    {
      send_to_char("You've been punished by the gods.  You can't use 'shout' or 'god'.\n\r",ch);
      return;
    }
  send_to_char("You seeks the gods for help.\n\r", ch); 
  
  act("$n seeks the gods for help!", FALSE, ch, 0, 0, TO_ROOM);
  strcpy(buffer,"<-- seeks the gods for help!");
  do_wiz(ch,buffer,0);
}
...
void do_sacrifice(struct char_data *ch, char *argument, int cmd)
{
	...
	if (GET_LEVEL(ch)>=21)
	  {
			send_to_char("You are a god! Just eat it.\n\r", ch); 
			return;
	  }
	if (!strcmp(arg1,"all"))
	  {
	    for (obj_object = ch->carrying;obj_object;obj_object=nextobj)
	      {
		nextobj = obj_object->next_content;
		if (CAN_SEE_OBJ(ch,obj_object))

		  {
		    if (IS_SET(obj_object->obj_flags.extra_flags,ITEM_NODROP))
		      {
			sprintf(buffer,"%s: It is cursed.\n\r",fname(obj_object->name));
			send_to_char(buffer, ch); 
		      }
		    else
		      { 
			sprintf(buffer,"The gods take the %s away!\n\r",fname(obj_object->name));
			send_to_char(buffer, ch); 
			sprintf(buffer,"$n sacrifices a %s.",fname(obj_object->name));
		act(buffer, FALSE, ch, 0, 0, TO_ROOM);
			sprintf(buffer,"<-- Sacrificed %s to the gods.",fname(obj_object->name));
			do_wiz(ch,buffer,0);
			obj_from_char(obj_object);
			extract_obj(obj_object);
			gain_exp(ch,1);
...
if (obj_object = get_obj_in_list_vis(ch, arg1, ch->carrying))
  {
    if (CAN_SEE_OBJ(ch,obj_object))
    ...
    else send_to_char("You can't see it in your possession.\n\r",ch);
  } else send_to_char("You don't find that in your possession.\n\r",ch);
}
...
bool test = FALSE;
	...
	if(is_number(arg))
	{
		...
		if(amount<0)
		{
			send_to_char("Your value was negative!\n\r",ch);
			return;
		}
		...
		tmp_object = get_obj_in_list_vis(ch, "gold", 
						 world[ch->in_room].contents);
		if (tmp_object)
		  {
		    if((GET_ITEM_TYPE(tmp_object)==ITEM_MONEY) &&
		       (tmp_object->item_number == -1))
		      {
			tmp_object->obj_flags.value[0]+=amount;
			free(tmp_object->name);
			tmp_object->name=strdup("coins gold");
			free(tmp_object->short_description);
			tmp_object->short_description=strdup("gold coins");
			free(tmp_object->description);
			tmp_object->description=strdup("A pile of gold coins.");
			sprintf(buffer,"You drop %d gold and add it to the pile on the ground here.\n\r",amount);
			send_to_char(buffer,ch);
