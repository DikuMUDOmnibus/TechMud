 ...
 void do_quiet(struct char_data *ch, char *argument, int cmd)
{
	...
	if (*buf)
		send_to_char("You can't force other people to listen or not listen.\n\t", ch);
	else if (IS_SET(ch->specials.act, PLR_QUIET))
	{
		send_to_char("You will listen to 'shout's, 'tell's, 'god's, 'sacrifice's, and devices again.\n\r", ch);
		REMOVE_BIT(ch->specials.act, PLR_QUIET);
	}
	else
	{
		send_to_char("You will now ignore 'shout's, 'tell's, 'god's, 'sacrifice's, and comm devices.\n\r", ch);
 ...
 void do_graphics(struct char_data *ch, char *argument, int cmd)
{
	...
	{
		send_to_char("Graphics now off.\n\r", ch);
		REMOVE_BIT(ch->specials.act, PLR_GRAPHICS);
	}
	else
	{
		send_to_char("Graphics now on.\n\r", ch);
		SET_BIT(ch->specials.act, PLR_GRAPHICS);
	}
}
 ...
 void do_godinvis(struct char_data *ch, char *argument, int cmd)
{
	...
	one_argument(argument, buf);
	if (*buf)
		send_to_char("You can't force other people to be god invisible or not!\n\t", ch);
	else if (IS_SET(ch->specials.act, PLR_GODINVIS))
	{
		send_to_char("You are now visible to all.\n\r", ch);

		REMOVE_BIT(ch->specials.act, PLR_GODINVIS);
	}
	else
	{
		send_to_char("You are now invisible to those whose levels are lower than you.\n\r", ch);
		SET_BIT(ch->specials.act, PLR_GODINVIS);
	}
}
 ...
 void do_prompt(struct char_data *ch, char *argument, int cmd)
{
	...
	one_argument(argument, buf);
	if (*buf)
		send_to_char("No options are supported.  Either all on or off.\n\r", ch);
	else if (IS_SET(ch->specials.act, PLR_PROMPT))
	{
		send_to_char("You have no prompt now.\n\r", ch);
		REMOVE_BIT(ch->specials.act, PLR_PROMPT);
	}
	else
	{
		send_to_char("You have prompt now.\n\r", ch);
		SET_BIT(ch->specials.act, PLR_PROMPT);
	}
}
 ...
 void do_aggressive(struct char_data *ch, char *argument, int cmd)
{
	...
	{
		send_to_char("You no longer auto-attack aggressive monsters.\n\r", ch);
		REMOVE_BIT(ch->specials.act, PLR_AGGRESSIVE);
	}
	else
	{
		send_to_char("You now auto-attack aggressive monsters.\n\r", ch);
		SET_BIT(ch->specials.act, PLR_AGGRESSIVE);
	}
}
	...
	if (*buf)
	  {
	    if (!is_number(buf))
	      {
		send_to_char("Input is not a number.\n\r",ch);
		return;
	      }
	    if ((atoi(buf)<0)||(atoi(buf)>GET_MAX_HIT(ch)))
	      {
	    ...
	    ch->specials.wimpy=atoi(buf);
	    if (IS_SET(ch->specials.act,PLR_WIMPY))
	      {
		sprintf(buf2,"Wimpy now set to %d.  Wimpy is currently ON.\n\r",atoi(buf));
		send_to_char(buf2,ch);
	      }
	    else
	      {
		sprintf(buf2,"Wimpy now set to %d.  Wimpy is currently OFF.\n\r",atoi(buf));
		send_to_char(buf2,ch);
	      }
	    
	  }

	else if (IS_SET(ch->specials.act, PLR_WIMPY))
	{
	  sprintf(buf,"Wimpy now OFF.  If set, you flee at %d hit points.\n\r",ch->specials.wimpy);
		send_to_char(buf, ch);
		REMOVE_BIT(ch->specials.act, PLR_WIMPY);
	}
	else
	{
	  sprintf(buf,"Wimpy now ON.  You flee at %d hit points.\n\r",ch->specials.wimpy);
		send_to_char(buf, ch);
