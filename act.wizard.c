...
int obj_virtual(int realnumber)
{

  return (obj_index[realnumber].virtual);
}

int mob_virtual(int realnumber)
{

  return (mob_index[realnumber].virtual);
}
...
if (IS_SET(ch->specials.act,PLR_GODINVIS))
act("You feel a loss of energy in the room.", FALSE, ch, 0, 0, TO_ROOM);
else act("$n disappears in a puff of smoke.", FALSE, ch, 0, 0, TO_ROOM);
	char_from_room(ch);
	char_to_room(ch, location);
if (IS_SET(ch->specials.act,PLR_GODINVIS))
act("You sense a splurge of energy in the room.", FALSE, ch, 0, 0, TO_ROOM);
else act("$n appears with an ear-splitting bang.", FALSE, ch, 0,0,TO_ROOM);
	do_look(ch, "",15);
}
...
do_noshout(struct char_data *ch, char *argument, int cmd)
{
	...
	{
		send_to_char("You can use 'shout' and 'god' commands again.\n\r", vict);
		send_to_char("NOSHOUT removed.\n\r", ch);
		REMOVE_BIT(vict->specials.act, PLR_NOSHOUT);
	}
	else
	{
		send_to_char("The gods take away your ability to 'shout' and ask 'god's for help!\n\r", vict);
		send_to_char("NOSHOUT set.\n\r", ch);
		SET_BIT(vict->specials.act, PLR_NOSHOUT);
	}
}

do_award(struct char_data *ch, char *argument, int cmd)
{
	struct char_data *vict;
	struct obj_data *dummy;
	char buf[MAX_INPUT_LENGTH];
	...
	if (!*buf)
		...
		act("$E should be awarding you... Not the other way around.", 0, ch, 0, vict, TO_CHAR);
	else if (IS_SET(vict->specials.act, PLR_AWARD))
	{
		send_to_char("The gods take away your award status before you got your gifts!\n\r", vict);

		send_to_char("AWARD removed.\n\r", ch);
		REMOVE_BIT(vict->specials.act, PLR_AWARD);
	}
	else
	{
		send_to_char("You have been awarded for you deeds.  Go to altar and pray for your reward.\n\r", vict);

		send_to_char("AWARD set.\n\r", ch);
		SET_BIT(vict->specials.act, PLR_AWARD);
	}
}

do_newbie(struct char_data *ch, char *argument, int cmd)
{
	struct char_data *vict;
	struct obj_data *dummy;
	char buf[MAX_INPUT_LENGTH];
	...
	if (!*buf)
		...
		send_to_char("Can't do that to a beast.\n\r", ch);
	else if (GET_LEVEL(vict) > 1)
		act("That person is greater than level 1.", 0, ch, 0, 0, TO_CHAR);
	else if (IS_SET(vict->specials.act, PLR_NEWBIE))
	{
		send_to_char("The gods take away your newbie status before you got your newbie package.\n\r", vict);

		send_to_char("NEWBIE removed.\n\r", ch);
		REMOVE_BIT(vict->specials.act, PLR_NEWBIE);
	}
	else
	{
		send_to_char("You have been offered a newbie package!  Go to altar and pray for your them.\n\r", vict);

		send_to_char("NEWBIE set.\n\r", ch);
		SET_BIT(vict->specials.act, PLR_NEWBIE);
	}
}
...
void do_pardon(struct char_data *ch, char *argument, int cmd)
{
	...
	{
		REMOVE_BIT(vict->specials.act, PLR_THIEF);
		REMOVE_BIT(vict->specials.act, PLR_KILLER);
		do_title(vict,"has been pardoned by the gods.",0);
		send_to_char("You have been pardoned for being a thief, killer, or both.\n\r", vict);
...
#define MAX_PORTAL_TIME 1
void do_portal(struct char_data *ch,char *argument,int cmd)
{
	...
	one_argument(argument, loc_str);
	if (!*loc_str)
	{
		send_to_char("You must supply a room number or a name where the portal is going to.\n\r", ch);
		return;
	}
	...
	if (isdigit(*loc_str))
	{
		...
		{
			send_to_char("The object does not exist.\n\r", ch);
			return;
		}
	else
	{
	...
	if (ch->in_room == location)
	  {
	    send_to_char("That would be to this same room!.\n\r",ch);
	    return;
	  }
	...
	CREATE(portal, struct obj_data, 1);
	clear_object(portal);

	
	portal->item_number = NOWHERE;
	portal->in_room = NOWHERE;
	portal->name = strdup("portal");
	portal->description = strdup("A portal is standing here.");
	portal->short_description = strdup("the portal");

	portal->contains = NULL;

	portal->obj_flags.type_flag = ITEM_PORTAL;
	portal->obj_flags.wear_flags = 0;
	portal->obj_flags.value[0] = world[location].number; /* teleport to */
	portal->obj_flags.value[1] = 1; /* dissapear? yes */
	portal->obj_flags.weight = 500;
	portal->obj_flags.cost_per_day = 500;
	portal->obj_flags.timer = MAX_PORTAL_TIME;

	portal->next = object_list;
	object_list = portal;

	
	obj_to_room(portal, ch->in_room);

	act("A portal appears from a bright light!", FALSE, ch, 0, 0, TO_ROOM);
	act("You create a portal in this room.", FALSE, ch, 0, 0, TO_CHAR);
	...
	portal->obj_flags.type_flag = ITEM_PORTAL;
	portal->obj_flags.wear_flags = 0;
	portal->obj_flags.value[0] = world[ch->in_room].number; /* teleport to */
	portal->obj_flags.value[1] = 1; /* dissapear? yes */
	portal->obj_flags.weight = 500;
	portal->obj_flags.cost_per_day = 500;
	portal->obj_flags.timer = MAX_PORTAL_TIME;
	...
	act("A portal appears from a bright light!", FALSE, ch, 0, 0, TO_ROOM);
	act("You create another portal at the other room.", FALSE, ch, 0, 0, TO_CHAR);
	
	char_from_room(ch);
	char_to_room(ch, old_loc);
}
