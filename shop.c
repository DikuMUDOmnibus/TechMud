	 ...
	 struct char_data *keeper, int shop_nr)
{
	...
	if(!(*argm))
	{
		...
		,GET_NAME(ch));
		do_ask(keeper,buf,19);
		return;
	}
	if( temp1 = 
	   get_obj_in_list(argm,keeper->carrying))
	  {
	    if (GET_ITEM_TYPE(temp1)!=ITEM_MODIFY)
	      {
	...
	if (GET_ITEM_TYPE(temp1)==ITEM_MODIFY)
	  {
	    if (GET_LEVEL(ch)>20)
	      {
		send_to_char("Deities do not need these services.\n\r",ch);
		return;
	      }
	    switch(temp1->obj_flags.value[0])
	      {

	      case 0:
 		tempint=MIN(GET_GOLD(ch)/temp1->obj_flags.cost,GET_MAX_HIT(ch)-GET_HIT(ch));
		if (!tempint)
		  {
		    send_to_char("You don't need any more!\n\r",ch);
		    return;
		  }
		GET_HIT(ch) += tempint;
		GET_GOLD(ch) -= (tempint * temp1->obj_flags.cost);
		GET_GOLD(keeper) += (tempint * temp1->obj_flags.cost);
		sprintf(buf,"You have been healed %d points, costing %d gold.\n\r",tempint,(tempint * temp1->obj_flags.cost));
		send_to_char(buf,ch);
		update_pos(ch);
		break;
	      case 1:
 		tempint=MIN(GET_GOLD(ch)/temp1->obj_flags.cost,GET_MAX_MANA(ch)-GET_MANA(ch));
		if (!tempint)
		  {
		    send_to_char("You don't need any more!\n\r",ch);
		    return;
		...
		GET_GOLD(ch) -= (tempint * temp1->obj_flags.cost);
		GET_GOLD(keeper) += (tempint * temp1->obj_flags.cost);
		sprintf(buf,"You have been manatized %d points, costing %d gold.\n\r",tempint,(tempint * temp1->obj_flags.cost));
		send_to_char(buf,ch);


		break;
	      case 2:
 		tempint=MIN(GET_GOLD(ch)/temp1->obj_flags.cost,GET_MAX_MOVE(ch)-GET_MOVE(ch));
		if (!tempint)
		  {
		    send_to_char("You don't need any more!\n\r",ch);
		    return;
		...
		GET_GOLD(ch) -= (tempint * temp1->obj_flags.cost);
		GET_GOLD(keeper) += (tempint * temp1->obj_flags.cost);
		sprintf(buf,"You have been energized %d points, costing %d gold.\n\r",tempint,(tempint * temp1->obj_flags.cost));
		send_to_char(buf,ch);
		...
		if (affected_by_spell(ch,SPELL_POISON))
		  ...
		  {
		    send_to_char("You are not poisoned!\n\r",ch);
		    return;
		  }
		...
		send_to_char("You have been cured of poison.\n\r",ch);
		break;
	      case 4:
		...
		send_to_char("You have been cured of blindness.\n\r",ch);
		break;
	      case 10:
		if (GET_EXP(ch) < (temp1->obj_flags.cost_per_day))
		  {
		    send_to_char("You don't have that much exp to trade.\n\r",ch);
		    return;
		  }

 		tempint=MIN(GET_GOLD(ch)/temp1->obj_flags.cost,(18 - ch->abilities.str));
		tempint=MIN(tempint,GET_EXP(ch)/temp1->obj_flags.cost_per_day);
		if (!tempint)
		  {
		    send_to_char("You don't need any more.\n\r",ch);
		    return;
		...
		ch->abilities.str += tempint;
		ch->tmpabilities.str += tempint;
		GET_EXP(ch) -= (temp1->obj_flags.cost_per_day);
		GET_GOLD(ch) -= (tempint * temp1->obj_flags.cost);
		GET_GOLD(keeper) += (tempint * temp1->obj_flags.cost);
		sprintf(buf,"Your strength gained %d points, costing %d gold, and %d exp.\n\r",tempint,(tempint * temp1->obj_flags.cost),temp1->obj_flags.cost_per_day);
		send_to_char(buf,ch);
		...
		if (GET_EXP(ch) < (temp1->obj_flags.cost_per_day))
 		...
 		tempint=MIN(GET_GOLD(ch)/temp1->obj_flags.cost,(18 - ch->abilities.dex));
		tempint=MIN(tempint,GET_EXP(ch)/temp1->obj_flags.cost_per_day);
		if (!tempint)
		  {
		    send_to_char("You don't need any more.\n\r",ch);
		...
		ch->abilities.dex += tempint;
		ch->tmpabilities.dex += tempint;
		GET_EXP(ch) -= (temp1->obj_flags.cost_per_day);
		GET_GOLD(ch) -= (tempint * temp1->obj_flags.cost);
		GET_GOLD(keeper) += (tempint * temp1->obj_flags.cost);
		sprintf(buf,"Your dexterity gained %d points, costing %d gold, and %d exp.\n\r",tempint,(tempint * temp1->obj_flags.cost),temp1->obj_flags.cost_per_day);
		send_to_char(buf,ch);
		...
		if (GET_EXP(ch) < (temp1->obj_flags.cost_per_day))
 		...
 		tempint=MIN(GET_GOLD(ch)/temp1->obj_flags.cost,(18 - ch->abilities.intel));
		tempint=MIN(tempint,GET_EXP(ch)/temp1->obj_flags.cost_per_day);
		if (!tempint)
		  {
		    send_to_char("You don't need any more.\n\r",ch);
		...
		ch->abilities.intel += tempint;
		ch->tmpabilities.intel += tempint;
		GET_EXP(ch) -= (temp1->obj_flags.cost_per_day);
		GET_GOLD(ch) -= (tempint * temp1->obj_flags.cost);
		GET_GOLD(keeper) += (tempint * temp1->obj_flags.cost);
		sprintf(buf,"Your intelligence gained %d points, costing %d gold, and %d exp.\n\r",tempint,(tempint * temp1->obj_flags.cost),temp1->obj_flags.cost_per_day);
		send_to_char(buf,ch);
		...
		if (GET_EXP(ch) < (temp1->obj_flags.cost_per_day))
 		...
 		tempint=MIN(GET_GOLD(ch)/temp1->obj_flags.cost,(18 - ch->abilities.wis));
		tempint=MIN(tempint,GET_EXP(ch)/temp1->obj_flags.cost_per_day);
		if (!tempint)
		  {
		    send_to_char("You don't need any more.\n\r",ch);
		...
		ch->abilities.wis += tempint;
		ch->tmpabilities.wis += tempint;
		GET_EXP(ch) -= (temp1->obj_flags.cost_per_day);
		GET_GOLD(ch) -= (tempint * temp1->obj_flags.cost);
		GET_GOLD(keeper) += (tempint * temp1->obj_flags.cost);
		sprintf(buf,"Your wisdom gained %d points, costing %d gold, and %d exp.\n\r",tempint,(tempint * temp1->obj_flags.cost),temp1->obj_flags.cost_per_day);
		send_to_char(buf,ch);
		...
		if (GET_EXP(ch) < (temp1->obj_flags.cost_per_day))
 		...
 		tempint=MIN(GET_GOLD(ch)/temp1->obj_flags.cost,(18 - ch->abilities.con));
		tempint=MIN(tempint,GET_EXP(ch)/temp1->obj_flags.cost_per_day);
		if (!tempint)
		  {
		    send_to_char("You don't need any more.\n\r",ch);
		...
		ch->abilities.con += tempint;
		ch->tmpabilities.con += tempint;
		GET_EXP(ch) -= (temp1->obj_flags.cost_per_day);
		GET_GOLD(ch) -= (tempint * temp1->obj_flags.cost);
		GET_GOLD(keeper) += (tempint * temp1->obj_flags.cost);
		sprintf(buf,"Your constitution gained %d points, costing %d gold, and %d exp.\n\r",tempint,(tempint * temp1->obj_flags.cost),temp1->obj_flags.cost_per_day);
		send_to_char(buf,ch);
		...
		GET_EXP(ch) -= (temp1->obj_flags.cost_per_day);
		GET_GOLD(ch) -= temp1->obj_flags.cost;
		GET_GOLD(keeper) += temp1->obj_flags.cost;

		GET_COND(ch,FULL) = -1;
		send_to_char("You are operated upon... Done.\n\r",ch);
		break;
	      case 16:
		if (GET_EXP(ch) < (temp1->obj_flags.cost_per_day))
		  {
	 ...
	 struct char_data *keeper,int shop_nr)
{
	...
	if ((get_obj_in_list_num(temp1->item_number,keeper->carrying)) || 
...
if ( (CAN_SEE_OBJ(ch,temp1) || (GET_ITEM_TYPE(temp1)==ITEM_MODIFY)) && 
     (temp1->obj_flags.cost>0) )
...
if(temp1->obj_flags.type_flag != ITEM_DRINKCON) 
  {
    if (GET_ITEM_TYPE(temp1)==ITEM_MODIFY)
      sprintf(buf2,"%s (each pt. for %d gold and %d exp)\n\r",
	      (temp1->short_description), 
	      (int)(temp1->obj_flags.cost*shop_index[shop_nr].profit_buy),
	      temp1->obj_flags.cost_per_day);
    else sprintf(buf2,"%s for %d gold coins.\n\r"
		 ,(temp1->short_description)
		 ,(int)(temp1->obj_flags.cost*
			shop_index[shop_nr].profit_buy));
  }
...
if (GET_ITEM_TYPE(temp1)==ITEM_MODIFY)
  sprintf(buf2,"%s (each pt. for %d gold and %d exp)\n\r",buf3,
	  (int)(temp1->obj_flags.cost*shop_index[shop_nr].profit_buy),temp1->obj_flags.cost_per_day);
else sprintf(buf2,"%s for %d gold coins.\n\r",buf3,
	     (int)(temp1->obj_flags.cost*shop_index[shop_nr].profit_buy));
			      }
			strcat(buf,CAP(buf2));
	...
	switch(shop_index[shop_nr].temper2)
	{
		case 0:
			sprintf(buf,"%s, don't ever try that again!",
				GET_NAME(ch));
			do_say(keeper,buf,19);
			return;

		case 1:
			sprintf(buf,"%s, get out of my sight!",
				GET_NAME(ch));
			do_shout(keeper,buf,19);
			return;
