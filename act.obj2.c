...
void do_dip(struct char_data *ch, char *argument, int cmd)
{
	...
	if(!*arg1) /* No arguments */
	{
		act("What do you want to dip?",FALSE,ch,0,0,TO_CHAR);
		return;
	}

	if(!(from_obj = get_obj_in_list_vis(ch,arg1,ch->carrying)))
	{
		act("You can't find it in your possesion!",FALSE,ch,0,0,TO_CHAR);
		return;
	}

	if(from_obj->obj_flags.type_flag!=ITEM_DRINKCON)
	{
		act("You can't dip that!",FALSE,ch,0,0,TO_CHAR);
		return;
	}
	...
	if(!*arg2)
	{
		act("What do you want to dip it into?",FALSE,ch,0,0,TO_CHAR);
		return;
	}

if(!(to_obj = get_obj_in_list_vis(ch,arg2,world[ch->in_room].contents)))
{
  act("You can't find it in the room!",FALSE,ch,0,0,TO_CHAR);
  return;
}

	if(to_obj->obj_flags.type_flag!=ITEM_DRINKCON)
	{
		act("You can't dip anything into that.",FALSE,ch,0,0,TO_CHAR);
		return;
	}

if(CAN_WEAR(to_obj,ITEM_TAKE))
{
  act("That thing can be picked up and poured.",FALSE,ch,0,0,TO_CHAR);
  return;
}

	if((from_obj->obj_flags.value[1]!=0)&&
		(from_obj->obj_flags.value[2]!=to_obj->obj_flags.value[2]))
	{
		act("Your container already has another type of liquid in it!",FALSE,ch,0,0,TO_CHAR);
		return;
	}

	if(!(from_obj->obj_flags.value[1]<from_obj->obj_flags.value[0]))
	{
		act("Your container is already full.",FALSE,ch,0,0,TO_CHAR);
		return;
	}

	sprintf(buf,"You dip the %s into the %s.",
		drinks[from_obj->obj_flags.value[2]],arg2);
	send_to_char(buf,ch);

	/* New alias */
	if (from_obj->obj_flags.value[1]==0) 
		name_to_drinkcon(from_obj,to_obj->obj_flags.value[2]);
	...
	/* Then how much to pour */
	/* if to_obj can contail it all give it all */
	amount = from_obj->obj_flags.value[0]-from_obj->obj_flags.value[1];;
