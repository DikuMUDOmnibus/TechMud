...
void do_order(struct char_data *ch, char *argument, int cmd)
{
 ...
 if ((!victim) && str_cmp("follower", name) && str_cmp("followers", name))
   {
 ...
 if (ch == victim)
  ...
  for (k = ch->followers; k; k = k->next)
	...
	{
	  send_to_char("One of your followers is not in this room.\n\r",ch);
	  continue;
	}
      if (!IS_AFFECTED(k->follower, AFF_CHARM))
	{
	  act("$n has an indifferent look.", FALSE, k->follower, 0, 0, TO_ROOM);
	  continue;
	}
      
      found = TRUE;
      if (saves_spell(k->follower,SAVING_SPELL))
	{
	  if (saves_spell(k->follower,SAVING_SPELL)&&
	      saves_spell(k->follower,SAVING_SPELL)&&
	      saves_spell(k->follower,SAVING_SPELL))
	    {
...
void do_assist(struct char_data *ch, char *argument, int cmd)
{
  ...
  if (!*name)
    {
      send_to_char("Who do you wish to assist in fighting?\n\r",ch);
      return;
    }
  ...
  if (victim == ch) {
    send_to_char("You are already assisting yourself...\n\r", ch);
    return;
  }
  if (!victim->specials.fighting)
    {
      send_to_char("That person is not fighting anyone at the moment.\n\r",ch);
      return;
    }
