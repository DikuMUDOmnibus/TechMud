...
if (GET_POS(victim)<=POSITION_DEAD)
  {
log("Error: damaging victim whose already dead");
return;
}
	...
	switch (GET_POS(victim)) {
		...
		default:  /* >= POSITION SLEEPING */
			...
			if ((!IS_NPC(victim))&&(IS_SET(victim->specials.act,PLR_WIMPY))) 
			  {
			    if (GET_HIT(victim)<victim->specials.wimpy)
			      do_flee(victim, "", 0);
			  }
			break;		
	}
...
void hit(struct char_data *ch, struct char_data *victim, int type)
{
	...
	if ((!IS_NPC(victim))&&(!IS_NPC(ch)))
	  {
	    sprintf(buffer,"%s attacked %s!",GET_NAME(ch),GET_NAME(victim));
	    log(buffer);

	    if ((!IS_SET(victim->specials.act,PLR_KILLER))&&
		(!IS_SET(victim->specials.act,PLR_THIEF)) )
	      {
		if (GET_LEVEL(ch)>GET_LEVEL(victim))
		  {
		    send_to_char("You can only attack players at equal or higher level than you.\n\r",ch);
		    return;
		  }
