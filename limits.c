	...
	if (!IS_NPC(ch))
	  {
	    ...
	    max = ch->points.max_mana +(graf(age(ch).year, 0,20,45,80,130,190,250));
	    else max = (graf(age(ch).year, 0,20,45,80,130,190,250));
	  }
	else 
	  max = 100;
...
void advance_level(struct char_data *ch)
{
	...
	switch(GET_CLASS(ch)) {
		...
		case CLASS_MAGIC_USER : {
			add_hp += number(3, 8);
			ch->points.max_mana += number(10,20);
		} break;
...
void set_title(struct char_data *ch)
{
  if ((IS_SET(ch->specials.act,PLR_KILLER))||
      (IS_SET(ch->specials.act,PLR_THIEF))) return;
...
for(j = object_list; j ; j = next_thing)
  {
    ...
    if (GET_ITEM_TYPE(j)==ITEM_TIME)
      {
	if (CAN_WEAR(j,ITEM_TAKE))
	  {

	    if (j->equipted_by)
	      act("You hear $p beep.",FALSE,j->equipted_by,j,0,TO_CHAR);
	    else if (j->carried_by)
	      act("You hear $p beep.",FALSE,j->carried_by,j,0,TO_CHAR);
	  }
	else
	  {
	      if ((j->in_room != NOWHERE) && (world[j->in_room].people))
		{
		  act("You hear $p sound DONG!",
    ...
    /* If this is a portal */
    if ( (GET_ITEM_TYPE(j) == ITEM_PORTAL) && (j->obj_flags.value[1]) ) 
      {
	/* timer count down */
	if (j->obj_flags.timer > 0) j->obj_flags.timer--;
	if (!j->obj_flags.timer) 
	  {
	    if ((j->in_room != NOWHERE) && (world[j->in_room].people))
		{
		  act("Suddenly, $p dissolves in a bright light.",
		      TRUE, world[j->in_room].people, j, 0, TO_ROOM);
		  act("Suddenly, $p dissolves in a bright light.",
		      TRUE, world[j->in_room].people, j, 0, TO_CHAR);
		}
